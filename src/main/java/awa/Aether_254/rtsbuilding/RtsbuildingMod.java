...
import awa.Aether_254.rtsbuilding.common.RtsBlocks;
import awa.Aether_254.rtsbuilding.common.RtsCreativeTabs;
...
import awa.Aether_254.rtsbuilding.server.feedback.RtsDamageFeedbackManager;
import awa.Aether_254.rtsbuilding.server.history.ServerHistoryManager;
...
import awa.Aether_254.rtsbuilding.server.service.*;
import awa.Aether_254.rtsbuilding.server.service.page.RtsStoragePageRequestCoalescer;
import awa.Aether_254.rtsbuilding.server.service.placement.RtsPlacementSound;
import awa.Aether_254.rtsbuilding.server.storage.cache.RtsEndpointLeaseCache;
...
import awa.Aether_254.rtsbuilding.server.tracking.RtsBlockTrackingEvents;
import awa.Aether_254.rtsbuilding.server.workflow.core.RtsWorkflowEngine;
import awa.Aether_254.rtsbuilding.network.RtsPayloadRegistrar;
 ...
 * 保持主类的职责清晰专注。</p>
 */
public class RtsbuildingMod implements ModInitializer {
    ...
    public void onInitialize() {
        try {
            ...
            // 如果文件不存在，会使用默认值并创建文件
            net.neoforged.neoforge.common.ModConfigSpec.load(Config.SPEC);
            net.neoforged.neoforge.common.ModConfigSpec.load(Config.CLIENT_SPEC);
            net.neoforged.neoforge.common.ModConfigSpec.load(Config.SERVER_SPEC);
        ...
        commonSetup(null);
        RtsPayloadRegistrar.register(new net.neoforged.neoforge.network.event.RegisterPayloadHandlersEvent());
        RtsBlockTrackingEvents.register();
        CommandRegistrationCallback.EVENT.register((dispatcher, registryAccess, environment) -> {
            RtsDeveloperScenarioCommand.register(dispatcher);
            RtsGuiCompatSetupCommand.register(dispatcher);
        });

        ServerLifecycleEvents.SERVER_STARTING.register(this::onServerStarting);
        ServerLifecycleEvents.SERVER_STARTED.register(GameEvents::onServerStarted);
        ...
        ServerEntityWorldChangeEvents.AFTER_PLAYER_CHANGE_WORLD.register((player, origin, destination) ->
                GameEvents.onPlayerChangedDimension(player, origin));
        ServerChunkEvents.CHUNK_LOAD.register(GameEvents::onChunkLoad);
        ServerTickEvents.END_SERVER_TICK.register(GameEvents::onServerTick);
    }
    ...
    static class GameEvents {
         ...
         * @param event 玩家登录事件
         */
        static void onPlayerLogin(ServerPlayer serverPlayer) {
         ...
         * @param event 玩家登出事件
         */
        static void onPlayerLogout(ServerPlayer serverPlayer) {
            if (serverPlayer != null) {
                try {
         ...
         */
        static void onPlayerChangedDimension(ServerPlayer serverPlayer, ServerLevel origin) {
            if (serverPlayer != null) {
                // 停止相机（坐标已失效，需重新定位）
                RtsCameraManager.stopIfActive(serverPlayer);
                // 取消旧维度的寻路任务
                ...
                RtsEffectAccumulator.INSTANCE.clearDimension(serverPlayer.getUUID(), origin.dimension());
            }
        }

        /** 仅唤醒等待这个 chunk 的任务，不扫描玩家或全服任务。 */
        static void onChunkLoad(ServerLevel level, net.minecraft.world.level.chunk.LevelChunk chunk) {
         ...
         */
        static void onPlayerTickPost(ServerPlayer serverPlayer) {
            if (serverPlayer != null) {
