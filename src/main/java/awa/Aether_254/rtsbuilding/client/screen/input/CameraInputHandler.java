...
import awa.Aether_254.rtsbuilding.client.screen.ultimine.UltimineUiAdapter;
...
import static awa.Aether_254.rtsbuilding.client.screen.standalone.BuilderScreenConstants.MIDDLE_CLICK_DRAG_THRESHOLD;
...
public final class CameraInputHandler {
    ...
    public static boolean isPrimaryActionMouse(int button) {
        return ClientKeyMappings.ACTION_PRIMARY.matchesMouse(mouseEvent(button));
    }

    public static boolean isBreakActionMouse(int button) {
        return ClientKeyMappings.ACTION_BREAK.matchesMouse(mouseEvent(button));
    }

    public static boolean isRotateDragActionMouse(int button) {
        return ClientKeyMappings.CAMERA_ROTATE_DRAG.matchesMouse(mouseEvent(button));
    }

    public static boolean isPanDragActionMouse(int button) {
        return ClientKeyMappings.CAMERA_PAN_DRAG.matchesMouse(mouseEvent(button));
    }

    public static boolean isKeyboardPanDragActionHeld() {
        return ClientKeyMappings.CAMERA_PAN_DRAG.isDown();
    }

    public static boolean isPickBlockActionMouse(int button) {
        return ClientKeyMappings.PICK_BLOCK.matchesMouse(mouseEvent(button));
    }

    private static net.minecraft.client.input.MouseButtonEvent mouseEvent(int button) {
    ...
    public boolean updateCameraVerticalHeldState(int keyCode, int scanCode, boolean down) {
        ...
        if (ClientKeyMappings.CAMERA_UP.matches(new net.minecraft.client.input.KeyEvent(keyCode, scanCode, 0))) {
            this.cameraUpActionHeld = down;
            handled = true;
        }
        if (ClientKeyMappings.CAMERA_DOWN.matches(new net.minecraft.client.input.KeyEvent(keyCode, scanCode, 0))) {
