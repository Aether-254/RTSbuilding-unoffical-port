...
public final class RtsSelectionNudge {
    ...
    public static Delta fromKey(int keyCode, int scanCode) {
        ...
        if (ClientKeyMappings.SELECTION_NUDGE_FORWARD.matches(new net.minecraft.client.input.KeyEvent(keyCode, scanCode, 0))
                || keyCode == GLFW.GLFW_KEY_KP_8) {
            return Delta.of(forward, step);
        }
        if (ClientKeyMappings.SELECTION_NUDGE_BACK.matches(new net.minecraft.client.input.KeyEvent(keyCode, scanCode, 0))
                || keyCode == GLFW.GLFW_KEY_KP_2) {
            return Delta.of(forward, -step);
        }
        if (ClientKeyMappings.SELECTION_NUDGE_LEFT.matches(new net.minecraft.client.input.KeyEvent(keyCode, scanCode, 0))
                || keyCode == GLFW.GLFW_KEY_KP_4) {
            return Delta.of(right, -step);
        }
        if (ClientKeyMappings.SELECTION_NUDGE_RIGHT.matches(new net.minecraft.client.input.KeyEvent(keyCode, scanCode, 0))
                || keyCode == GLFW.GLFW_KEY_KP_6) {
            return Delta.of(right, step);
        }
        if (ClientKeyMappings.SELECTION_NUDGE_UP.matches(new net.minecraft.client.input.KeyEvent(keyCode, scanCode, 0))) {
            return new Delta(0, step, 0);
        }
        if (ClientKeyMappings.SELECTION_NUDGE_DOWN.matches(new net.minecraft.client.input.KeyEvent(keyCode, scanCode, 0))) {
            return new Delta(0, -step, 0);
        }
        return null;
    }
