...
import static awa.Aether_254.rtsbuilding.client.screen.standalone.BuilderScreenConstants.TOP_H;
...
public final class BlueprintWindowPanel extends RtsWindowPanel {
    ...
    private boolean clickTextBox(WindowTextBox box, double mouseX, double mouseY, int button) {
        ...
        boolean clicked = box.mouseClicked(new net.minecraft.client.input.MouseButtonEvent(mouseX, mouseY, new net.minecraft.client.input.MouseButtonInfo(button, 0)), true);
        if (clicked) {
    ...
    private void clickButtons(double mouseX, double mouseY, int button, WindowButton... buttons) {
        for (WindowButton windowButton : buttons) {
            if (windowButton != null && windowButton.mouseClicked(new net.minecraft.client.input.MouseButtonEvent(mouseX, mouseY, new net.minecraft.client.input.MouseButtonInfo(button, 0)), true)) {
                return;
            }
        }
    }
    ...
    protected boolean handleWindowKeyPressed(int keyCode, int scanCode, int modifiers) {
        ...
        if (focused != null) {
            ...
            return focused.keyPressed(new net.minecraft.client.input.KeyEvent(keyCode, scanCode, modifiers));
        }
        if (BlueprintUiStateAdapter.snapshot().isCapture()) {
    ...
    protected boolean handleWindowCharTyped(char codePoint, int modifiers) {
        WindowTextBox focused = focusedTextBox();
        return focused != null && focused.charTyped(new net.minecraft.client.input.CharacterEvent(codePoint, modifiers));
    }
