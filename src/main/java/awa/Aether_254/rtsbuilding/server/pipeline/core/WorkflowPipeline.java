...
import awa.Aether_254.rtsbuilding.server.pipeline.tool.ToolLeaseRollbackPolicy;
    ...
    private static void rollbackIfNeeded(PipelineContext ctx) {
        ...
        if (rollback.cancelSubmittedTask() && ctx.hasData(PipelineContext.KEY_WORKFLOW_ENTRY_ID)) {
            ...
            var session = ctx.getData(
                    awa.Aether_254.rtsbuilding.server.pipeline.validation.SessionValidatePipe.KEY_SESSION);
            if (session != null) {
