...
public final class RtsSharedProgressionData extends SavedData {
    ...
    private static final Codec<RtsSharedProgressionData> CODEC =
            CompoundTag.CODEC.xmap(RtsSharedProgressionData::load, RtsSharedProgressionData::save);
    private static final SavedDataType<RtsSharedProgressionData> TYPE =
            new SavedDataType<>(DATA_NAME, RtsSharedProgressionData::new, CODEC, DataFixTypes.LEVEL);
    ...
    private static RtsSharedProgressionData load(CompoundTag tag) {
        ...
        for (int i = 0; i < groups.size(); i++) {
            ...
            for (int j = 0; j < plugins.size(); j++) {
                ...
                ItemStack stack = pluginTag.read(KEY_PLUGIN_STACK, ItemStack.CODEC).orElse(ItemStack.EMPTY);
                if (stack.isEmpty()) {
                    continue;
                }
                UUID owner = pluginTag.contains(KEY_PLUGIN_OWNER)
                        ? awa.Aether_254.rtsbuilding.common.nbt.RtsNbtUuid.get(pluginTag, KEY_PLUGIN_OWNER)
                        : null;
        ...
        for (var entry : this.groups.entrySet()) {
            ...
            if (!progression.plugins.isEmpty()) {
                ...
                for (SharedPlugin plugin : progression.plugins) {
                    ...
                    pluginTag.store(KEY_PLUGIN_STACK, ItemStack.CODEC, plugin.stack().copyWithCount(1));
                    pluginTag.putLong(KEY_PLUGIN_INSTALLED_GAME_TIME, plugin.installedGameTime());
                    if (plugin.ownerId() != null) {
                        awa.Aether_254.rtsbuilding.common.nbt.RtsNbtUuid.put(pluginTag, KEY_PLUGIN_OWNER, plugin.ownerId());
                    }
