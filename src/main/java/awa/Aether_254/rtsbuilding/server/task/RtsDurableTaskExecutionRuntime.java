...
final class RtsDurableTaskExecutionRuntime {
    ...
    private final Map<awa.Aether_254.rtsbuilding.server.task.identity.TaskId, MiningProgressOverlay>
    ...
    DurableTaskScheduler.TickStats tick(
            awa.Aether_254.rtsbuilding.server.task.persistence.TaskPersistenceCoordinator coordinator,
            java.util.Collection<awa.Aether_254.rtsbuilding.server.task.persistence.TaskSnapshot> candidates,
            long maxNanos,
            int maxUnits,
    ...
    awa.Aether_254.rtsbuilding.server.task.persistence.TaskSnapshot transitionMiningSnapshot(
            awa.Aether_254.rtsbuilding.server.task.persistence.TaskSnapshot snapshot,
            awa.Aether_254.rtsbuilding.server.task.persistence.TaskLifecycleState lifecycle,
            long gameTime) {
        ...
        MiningTaskPayload payload = overlay != null && overlay.baseRevision() == snapshot.revision()
                ? overlay.payload()
                : awa.Aether_254.rtsbuilding.server.task.mining.MiningTaskCodec.decode(snapshot.payload());
    ...
    awa.Aether_254.rtsbuilding.server.task.mining.MiningTaskState currentMiningState(
            awa.Aether_254.rtsbuilding.server.task.persistence.TaskSnapshot snapshot) {
        ...
        return awa.Aether_254.rtsbuilding.server.task.mining.MiningTaskCodec.decode(snapshot.payload()).state();
    }
    ...
    private DurableTaskScheduler.SliceResult executeDurablePlacement(
        ...
        if (payload.workflowEntryId() >= 0
                && awa.Aether_254.rtsbuilding.server.workflow.core.RtsWorkflowEngine.getInstance()
                        .from(player, payload.workflowEntryId()).isEmpty()) {
            var failed = snapshot.nextRevision(
                    awa.Aether_254.rtsbuilding.server.task.persistence.TaskLifecycleState.FAILED,
        ...
        var lifecycle = switch (result.outcome()) {
            case CONTINUE -> awa.Aether_254.rtsbuilding.server.task.persistence.TaskLifecycleState.RUNNING;
            case WAITING_RESOURCE ->
                    awa.Aether_254.rtsbuilding.server.task.persistence.TaskLifecycleState.WAITING_RESOURCE;
            case COMPLETE -> awa.Aether_254.rtsbuilding.server.task.persistence.TaskLifecycleState.COMPLETED;
        };
        awa.Aether_254.rtsbuilding.server.task.persistence.TaskWaitKey waitKey = null;
        if (result.outcome()
                == awa.Aether_254.rtsbuilding.server.task.placement.PlacementSliceResult.Outcome.WAITING_RESOURCE) {
            String itemId = result.state().definition().getStringOr("itemId", "");
            waitKey = new awa.Aether_254.rtsbuilding.server.task.persistence.TaskWaitKey(
    ...
    private DurableTaskScheduler.SliceResult executeDurableDestruction(
        ...
        DestructionTaskPayload payload = awa.Aether_254.rtsbuilding.server.task.destruction.DestructionTaskCodec
                .decode(snapshot.payload());
        var player = activeServer == null ? null : activeServer.getPlayerList().getPlayer(payload.ownerId());
        if (player == null || !player.level().dimension().equals(payload.dimension())) {
            return durableNoProgress(snapshot, payload.state().cursorUnits(), payload.state().succeededUnits(),
        ...
        if (awa.Aether_254.rtsbuilding.server.workflow.core.RtsWorkflowEngine.getInstance()
                .from(player, payload.workflowEntryId()).isEmpty()) {
            var failed = snapshot.nextRevision(
                    awa.Aether_254.rtsbuilding.server.task.persistence.TaskLifecycleState.FAILED,
                    null, player.level().getGameTime(), payload.state().cursorUnits(),
        ...
        var waitKey = result.outcome()
                == awa.Aether_254.rtsbuilding.server.task.destruction.DestructionSliceResult.Outcome.WAITING_RESOURCE
                ? new awa.Aether_254.rtsbuilding.server.task.persistence.TaskWaitKey(
                        "tool", "hotbar:" + Byte.toUnsignedInt(result.state().toolSlot()))
                : null;
        var next = snapshot.nextRevision(lifecycle, waitKey, player.level().getGameTime(),
                result.state().cursorUnits(), result.state().succeededUnits(), result.state().failedUnits(),
                awa.Aether_254.rtsbuilding.server.task.destruction.DestructionTaskCodec.encode(nextPayload));
        if (lifecycle.terminal()) {
            RtsDestructionBatch.recordDetachedHistory(player, result.state());
            projectDurableTerminal(player, next);
            boolean another = awa.Aether_254.rtsbuilding.server.task.persistence.TaskPersistenceRuntime.INSTANCE
    ...
    private void projectDurableTerminal(net.minecraft.server.level.ServerPlayer player,
        ...
        if (snapshot.state() == awa.Aether_254.rtsbuilding.server.task.persistence.TaskLifecycleState.COMPLETED) {
            token.complete();
        } else {
            token.cancel();
        }
    ...
    private DurableTaskScheduler.SliceResult executeDurableMining(
        ...
        if (result.outcome() == awa.Aether_254.rtsbuilding.server.task.mining.MiningSliceResult.Outcome.CONTINUE
                || result.outcome() == awa.Aether_254.rtsbuilding.server.task.mining.MiningSliceResult.Outcome.NEXT_TICK) {
            ...
            var checkpoint = snapshot.nextRevision(
                    awa.Aether_254.rtsbuilding.server.task.persistence.TaskLifecycleState.RUNNING,
                    null, player.level().getGameTime(), result.state().cursorUnits(),
                    result.state().succeededUnits(), result.state().failedUnits(),
                    awa.Aether_254.rtsbuilding.server.task.mining.MiningTaskCodec
        ...
        if (result.outcome() == awa.Aether_254.rtsbuilding.server.task.mining.MiningSliceResult.Outcome.WAITING) {
            miningProgressOverlays.put(snapshot.id(), new MiningProgressOverlay(
                    snapshot.revision(), payload.withState(result.state()), pendingUnits, lastCheckpointGameTime));
            return new DurableTaskScheduler.SliceResult(snapshot, result.processedUnits());
        }
        ...
        var lifecycle = switch (result.outcome()) {
            ...
            case WAITING -> result.waitHint().kind().equals("chunk")
                    ? awa.Aether_254.rtsbuilding.server.task.persistence.TaskLifecycleState.WAITING_CHUNK
                    : awa.Aether_254.rtsbuilding.server.task.persistence.TaskLifecycleState.WAITING_RESOURCE;
            case COMPLETE -> awa.Aether_254.rtsbuilding.server.task.persistence.TaskLifecycleState.COMPLETED;
        };
