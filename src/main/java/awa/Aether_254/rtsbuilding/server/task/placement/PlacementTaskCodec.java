...
public final class PlacementTaskCodec {
    ...
    public static PlacementTaskPayload decode(CompoundTag tag) {
        if (tag == null
                ...
                || !awa.Aether_254.rtsbuilding.common.nbt.RtsNbtUuid.has(tag, "owner")
        ...
        PlacementTaskState state = new PlacementTaskState(
                ...
                resumePolicy);
        return new PlacementTaskPayload(awa.Aether_254.rtsbuilding.common.nbt.RtsNbtUuid.get(tag, "owner"), dimension, workflow, state);
    }
    ...
    private static void validateDefinition(CompoundTag definition, int totalUnits) {
        ...
        int targets = definition.getLongArray("positions").map(values -> values.length).orElse(0);
