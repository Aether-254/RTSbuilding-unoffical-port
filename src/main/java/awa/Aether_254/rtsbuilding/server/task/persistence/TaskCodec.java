...
public final class TaskCodec {
    ...
    private static CompoundTag encodeAsset(TaskAssetMetadata metadata) {
        ...
        awa.Aether_254.rtsbuilding.common.nbt.RtsNbtUuid.put(tag, "asset_id", metadata.assetId().value());
        awa.Aether_254.rtsbuilding.common.nbt.RtsNbtUuid.put(tag, "task_id", metadata.taskId().value());
    ...
    private static TaskAssetMetadata decodeAsset(CompoundTag tag) {
        ...
        return new TaskAssetMetadata(
                new TaskAssetId(awa.Aether_254.rtsbuilding.common.nbt.RtsNbtUuid.get(tag, "asset_id")),
                new TaskId(awa.Aether_254.rtsbuilding.common.nbt.RtsNbtUuid.get(tag, "task_id")),
    ...
    private CompoundTag encodeSnapshotUnchecked(TaskSnapshot snapshot) {
        CompoundTag tag = new CompoundTag();
        awa.Aether_254.rtsbuilding.common.nbt.RtsNbtUuid.put(tag, "id", snapshot.id().value());
        awa.Aether_254.rtsbuilding.common.nbt.RtsNbtUuid.put(tag, "submission", snapshot.submissionId().value());
        awa.Aether_254.rtsbuilding.common.nbt.RtsNbtUuid.put(tag, "owner", snapshot.ownerId());
    ...
    public TaskSnapshot decodeSnapshot(CompoundTag tag) {
        ...
        return new TaskSnapshot(
                new TaskId(awa.Aether_254.rtsbuilding.common.nbt.RtsNbtUuid.get(tag, "id")),
                new SubmissionId(awa.Aether_254.rtsbuilding.common.nbt.RtsNbtUuid.get(tag, "submission")),
                awa.Aether_254.rtsbuilding.common.nbt.RtsNbtUuid.get(tag, "owner"),
                dimension,
                type,
                state,
                workflowEntryId,
    ...
    private CompoundTag encodeTombstone(TaskTombstone tombstone) {
        CompoundTag tag = new CompoundTag();
        awa.Aether_254.rtsbuilding.common.nbt.RtsNbtUuid.put(tag, "id", tombstone.taskId().value());
        awa.Aether_254.rtsbuilding.common.nbt.RtsNbtUuid.put(tag, "submission", tombstone.submissionId().value());
        awa.Aether_254.rtsbuilding.common.nbt.RtsNbtUuid.put(tag, "owner", tombstone.ownerId());
    ...
    private static ListTag requireList(CompoundTag root, String key, int elementType) {
        ...
        if (!list.isEmpty() && list.get(0).getId() != elementType) {
    ...
    private static void requireUuid(CompoundTag tag, String key) {
        if (!awa.Aether_254.rtsbuilding.common.nbt.RtsNbtUuid.has(tag, key)) throw new TaskCodecException("缺少 UUID 字段: " + key);
    }
