...
import awa.Aether_254.rtsbuilding.server.service.destruction.RtsDestructionBatch;
import awa.Aether_254.rtsbuilding.server.service.mining.RtsMiningStateMachine;
import awa.Aether_254.rtsbuilding.server.service.mining.RtsDropAbsorber;
import awa.Aether_254.rtsbuilding.server.service.placement.RtsPlacementBatch;
import awa.Aether_254.rtsbuilding.server.pipeline.blueprint.BlueprintTickPipe;
import awa.Aether_254.rtsbuilding.server.pipeline.context.BlueprintContext;
import awa.Aether_254.rtsbuilding.server.service.RtsPlacedRecoveryService;
...
public final class RtsTaskEngine {
    ...
    private final Map<awa.Aether_254.rtsbuilding.server.task.identity.TaskId,
    ...
    private final DurableBlueprintTaskBridge durableBlueprintBridge =
            new DurableBlueprintTaskBridge(this,
                    awa.Aether_254.rtsbuilding.server.task.persistence.TaskPersistenceRuntime.INSTANCE);
    ...
    public TaskScheduler.TickStats tick(MinecraftServer server) {
        ...
        var coordinator = awa.Aether_254.rtsbuilding.server.task.persistence.TaskPersistenceRuntime.INSTANCE
                .coordinator();
        ...
        java.util.LinkedHashMap<awa.Aether_254.rtsbuilding.server.task.identity.TaskId,
                awa.Aether_254.rtsbuilding.server.task.persistence.TaskSnapshot> durableCandidates =
    ...
    public TaskDiagnostics diagnostics(UUID ownerId) {
        ...
        for (var snapshot : awa.Aether_254.rtsbuilding.server.task.persistence.TaskPersistenceRuntime.INSTANCE
    ...
    public void checkpointAllDurableExecutions(MinecraftServer server) {
        durableRuntime.checkpointMiningExecutions(
                awa.Aether_254.rtsbuilding.server.task.persistence.TaskPersistenceRuntime.INSTANCE.coordinator(),
    ...
    public boolean setWorkflowPaused(net.minecraft.server.level.ServerPlayer player, int workflowEntryId,
        ...
        var workflow = awa.Aether_254.rtsbuilding.server.workflow.core.RtsWorkflowEngine.getInstance()
        ...
        if (durable != null && (durable.type() == TaskType.PLACEMENT
            ...
            var state = paused
                    ? awa.Aether_254.rtsbuilding.server.task.persistence.TaskLifecycleState.PAUSED
                    : awa.Aether_254.rtsbuilding.server.task.persistence.TaskLifecycleState.QUEUED;
    ...
    public void pauseAllWorkflowTasks(net.minecraft.server.level.ServerPlayer player) {
        if (player == null) return;
        for (var status : awa.Aether_254.rtsbuilding.server.workflow.core.RtsWorkflowEngine.getInstance()
    ...
    public boolean cancelWorkflowTask(net.minecraft.server.level.ServerPlayer player, int workflowEntryId) {
        ...
        if (durable != null && (durable.type() == TaskType.PLACEMENT
            ...
            if (durable.type() == TaskType.PLACEMENT) {
                RtsPlacementBatch.recordDetachedHistory(player,
                        awa.Aether_254.rtsbuilding.server.task.placement.PlacementTaskCodec
                                .decode(durable.payload()).state());
            } else if (durable.type() == TaskType.DESTRUCTION) {
                RtsDestructionBatch.recordDetachedHistory(player,
                        awa.Aether_254.rtsbuilding.server.task.destruction.DestructionTaskCodec
                                .decode(durable.payload()).state());
            } else {
            ...
            var cancelled = durable.type() == TaskType.MINING
                    ? durableRuntime.transitionMiningSnapshot(durable,
                            awa.Aether_254.rtsbuilding.server.task.persistence.TaskLifecycleState.CANCELLED,
        ...
        if (record != null) {
            ...
            if (record.payload() instanceof BlueprintTaskPayload) {
                awa.Aether_254.rtsbuilding.server.pipeline.blueprint.BlueprintPersistence
    ...
    public void resumeWaitingPlacementItems(net.minecraft.server.level.ServerPlayer player,
        ...
        for (String itemId : new java.util.LinkedHashSet<>(changedItemIds)) {
            ...
            var key = new awa.Aether_254.rtsbuilding.server.task.persistence.TaskWaitKey("item", itemId);
    ...
    public int resumeAllWaitingPlacements(net.minecraft.server.level.ServerPlayer player) {
        ...
        for (var snapshot : coordinator.query().ownedBy(player.getUUID())) {
            if (snapshot.type() != TaskType.PLACEMENT
                    ...
                    || snapshot.state() != awa.Aether_254.rtsbuilding.server.task.persistence.TaskLifecycleState.WAITING_RESOURCE) {
                continue;
            }
            coordinator.replace(snapshot.nextRevision(
                    awa.Aether_254.rtsbuilding.server.task.persistence.TaskLifecycleState.QUEUED,
    ...
    public PendingPlacementTaskView findWaitingPlacement(
        ...
        var snapshot = awa.Aether_254.rtsbuilding.server.task.persistence.TaskPersistenceRuntime.INSTANCE
                .coordinator().query().findByWorkflow(
                        player.getUUID(), player.level().dimension().identifier().toString(), workflowEntryId)
                .orElse(null);
        if (snapshot == null || snapshot.type() != TaskType.PLACEMENT
                || snapshot.state()
                != awa.Aether_254.rtsbuilding.server.task.persistence.TaskLifecycleState.WAITING_RESOURCE) {
            return null;
        }
        PlacementTaskPayload payload = awa.Aether_254.rtsbuilding.server.task.placement.PlacementTaskCodec
    ...
    public boolean resumeWaitingPlacementWithStrategy(
            ...
            awa.Aether_254.rtsbuilding.server.task.identity.TaskId expectedTaskId,
        ...
        coordinator.replace(snapshot.nextRevision(
                ...
                awa.Aether_254.rtsbuilding.server.task.placement.PlacementTaskCodec.encode(nextPayload)));
        return true;
    }

    public String firstPlacementItemId(net.minecraft.server.level.ServerPlayer player) {
        if (player == null) return "";
        return awa.Aether_254.rtsbuilding.server.task.persistence.TaskPersistenceRuntime.INSTANCE
                .coordinator().query().ownedBy(player.getUUID()).stream()
                .filter(snapshot -> snapshot.type() == TaskType.PLACEMENT && !snapshot.state().terminal())
                .map(snapshot -> awa.Aether_254.rtsbuilding.server.task.placement.PlacementTaskCodec
    ...
    public int activeMiningWorkflowEntry(net.minecraft.server.level.ServerPlayer player) {
        ...
        return awa.Aether_254.rtsbuilding.server.task.persistence.TaskPersistenceRuntime.INSTANCE
                ...
                .mapToInt(awa.Aether_254.rtsbuilding.server.task.persistence.TaskSnapshot::workflowEntryId)
    ...
    public boolean hasCommittedMiningBatch(net.minecraft.server.level.ServerPlayer player) {
        ...
        String dimensionId = player.level().dimension().identifier().toString();
        var states = awa.Aether_254.rtsbuilding.server.task.persistence.TaskPersistenceRuntime.INSTANCE
                .coordinator().query().ownedBy(player.getUUID()).stream()
                .filter(snapshot -> snapshot.type() == TaskType.MINING && !snapshot.state().terminal())
                .filter(snapshot -> snapshot.dimensionId().equals(dimensionId))
        ...
        return !states.isEmpty() && states.stream()
                .allMatch(awa.Aether_254.rtsbuilding.server.task.mining.MiningTaskState::committedBatch);
    }
    ...
    public int cancelActiveMiningTasks(net.minecraft.server.level.ServerPlayer player) {
        if (player == null) return 0;
        String dimensionId = player.level().dimension().identifier().toString();
        var workflowIds = awa.Aether_254.rtsbuilding.server.task.persistence.TaskPersistenceRuntime.INSTANCE
                .coordinator().query().ownedBy(player.getUUID()).stream()
                .filter(snapshot -> snapshot.type() == TaskType.MINING && !snapshot.state().terminal())
                .filter(snapshot -> snapshot.dimensionId().equals(dimensionId))
                .map(awa.Aether_254.rtsbuilding.server.task.persistence.TaskSnapshot::workflowEntryId)
                .filter(entryId -> entryId >= 0)
                .distinct().toList();
        int cancelled = 0;
    ...
    public int workflowTaskTotalUnits(
        ...
        return awa.Aether_254.rtsbuilding.server.task.persistence.TaskPersistenceRuntime.INSTANCE
                .coordinator().query().findByWorkflow(
                        player.getUUID(), player.level().dimension().identifier().toString(), workflowEntryId)
                .map(awa.Aether_254.rtsbuilding.server.task.persistence.TaskSnapshot::totalUnits)
                .orElse(0);
    }

    private void resumeAvailableToolWaits(
            net.minecraft.server.level.ServerPlayer player,
            awa.Aether_254.rtsbuilding.server.storage.session.RtsStorageSession session,
            awa.Aether_254.rtsbuilding.server.task.persistence.TaskPersistenceCoordinator coordinator) {
        if (player == null || session == null
                || (!player.isCreative()
                && !session.mining.miningSelectedToolRequested
                && (session.mining.miningToolLease == null
                || session.mining.miningToolLease.isEmpty()))
                || awa.Aether_254.rtsbuilding.server.service.mining.RtsMiningValidator
                        .isToolNearBreak(player, session)) return;
        resumeWaitKey(coordinator, player.getUUID(),
                new awa.Aether_254.rtsbuilding.server.task.persistence.TaskWaitKey(
    ...
    public void resumeLoadedChunk(net.minecraft.server.level.ServerLevel level,
        ...
        var runtime = awa.Aether_254.rtsbuilding.server.task.persistence.TaskPersistenceRuntime.INSTANCE;
        // GameTest/专用服务器会在 ServerStarting 之前提升出生区块；此时还没有恢复索引可唤醒。
        if (!runtime.isStarted()) return;
        var coordinator = runtime.coordinator();
        var key = new awa.Aether_254.rtsbuilding.server.task.persistence.TaskWaitKey(
    ...
    private static void resumeWaitKey(
            awa.Aether_254.rtsbuilding.server.task.persistence.TaskPersistenceCoordinator coordinator,
            UUID ownerId,
            awa.Aether_254.rtsbuilding.server.task.persistence.TaskWaitKey key,
            long gameTime) {
        for (var snapshot : coordinator.query().waitingFor(key)) {
    ...
    TaskRecord activateDurableBlueprint(
            awa.Aether_254.rtsbuilding.server.task.identity.TaskId taskId,
            awa.Aether_254.rtsbuilding.server.task.persistence.TaskSnapshot snapshot,
            BlueprintContext context,
            net.minecraft.resources.ResourceKey<net.minecraft.world.level.Level> sourceDimension) {
        int entryId = context.getData(
                awa.Aether_254.rtsbuilding.server.pipeline.core.PipelineContext.KEY_WORKFLOW_ENTRY_ID);
    ...
    public boolean submitPlacementJob(net.minecraft.server.level.ServerPlayer player,
         ...
         */
        var submission = awa.Aether_254.rtsbuilding.server.task.identity.SubmissionId.create();
        var taskId = awa.Aether_254.rtsbuilding.server.task.identity.TaskId
                .fromSubmission(player.getUUID(), submission);
        long gameTime = player.level().getGameTime();
        var snapshot = new awa.Aether_254.rtsbuilding.server.task.persistence.TaskSnapshot(
                ...
                awa.Aether_254.rtsbuilding.server.task.placement.PlacementTaskCodec.encode(payload));
    ...
    public boolean submitDestructionJob(net.minecraft.server.level.ServerPlayer player,
        ...
        var snapshot = new awa.Aether_254.rtsbuilding.server.task.persistence.TaskSnapshot(
                ...
                job.workflowEntryId(), null, 1L, gameTime, gameTime,
                job.totalCount(), job.getIndex(), job.successfulCount(), job.failedCount(),
                awa.Aether_254.rtsbuilding.server.task.destruction.DestructionTaskCodec.encode(payload));
        coordinator.submit(snapshot);
        return true;
    }
    ...
    public boolean submitMiningTargets(
        ...
        var state = new awa.Aether_254.rtsbuilding.server.task.mining.MiningTaskState(
                progressiveSingle
                        ? awa.Aether_254.rtsbuilding.server.task.mining.MiningTaskState.Mode.PROGRESSIVE_SINGLE
                        : awa.Aether_254.rtsbuilding.server.task.mining.MiningTaskState.Mode.BATCH,
    ...
    private boolean submitMiningState(net.minecraft.server.level.ServerPlayer player,
            awa.Aether_254.rtsbuilding.server.task.mining.MiningTaskState state) {
        ...
        var snapshot = new awa.Aether_254.rtsbuilding.server.task.persistence.TaskSnapshot(
                ...
                awa.Aether_254.rtsbuilding.server.task.mining.MiningTaskCodec.encode(payload));
        awa.Aether_254.rtsbuilding.server.task.persistence.TaskPersistenceRuntime.INSTANCE
                .coordinator().submit(snapshot);
        return true;
    }
    ...
    private TaskStepResult executeBlueprint(TaskRecord task, TaskBudget budget) {
        ...
        var token = awa.Aether_254.rtsbuilding.server.workflow.core.RtsWorkflowEngine.getInstance()
        ...
        if (!payload.ready()) {
            ...
            if (payload.ready() && payload.shouldCheckpoint(true)
                ...
                awa.Aether_254.rtsbuilding.server.pipeline.blueprint.BlueprintPersistence.saveToEntry(
    ...
    private void syncFunnelTask(net.minecraft.server.level.ServerPlayer player,
        ...
        boolean active = session.funnel.funnelEnabled
                && session.mode == awa.Aether_254.rtsbuilding.common.build.BuilderMode.FUNNEL;
        if (!active) {
    ...
    private void applyInitialPause(net.minecraft.server.level.ServerPlayer player, int workflowEntryId,
        ...
        var entry = awa.Aether_254.rtsbuilding.server.workflow.core.RtsWorkflowEngine.getInstance()
    ...
    private void projectDurableWorkflowLifecycles(MinecraftServer server,
            awa.Aether_254.rtsbuilding.server.task.persistence.TaskPersistenceCoordinator coordinator) {
        java.util.Set<awa.Aether_254.rtsbuilding.server.task.identity.TaskId> activeIds =
    ...
    private awa.Aether_254.rtsbuilding.server.workflow.core.RtsWorkflowToken workflowToken(
            TaskRecord record, int entryId) {
        var engine = awa.Aether_254.rtsbuilding.server.workflow.core.RtsWorkflowEngine.getInstance();
        var dimension = taskDimension(record);
        if (engine.findEntryByPlayer(record.ownerId(), dimension, entryId) == null) return null;
        return new awa.Aether_254.rtsbuilding.server.workflow.core.RtsWorkflowToken(
    ...
    private boolean isTaskEngineWorkflow(
            awa.Aether_254.rtsbuilding.server.workflow.model.RtsWorkflowType type) {
        return switch (type) {
    ...
    public record PendingPlacementTaskView(
            awa.Aether_254.rtsbuilding.server.task.identity.TaskId taskId,
            long revision,
            awa.Aether_254.rtsbuilding.server.task.placement.PlacementTaskState state) {
