...
public final class RtsPageSharedHelpers {
    ...
    public static boolean shouldIncludePlayerMainInventoryInStorageView(ServerPlayer player, RtsStorageSession session) {
        if (player == null || player.containerMenu instanceof awa.Aether_254.rtsbuilding.server.menu.RtsCraftTerminalMenu) {
            return false;
        }
        if (session != null && session.linkedStorageInfo.isEmpty() && !awa.Aether_254.rtsbuilding.compat.bd.RtsBdCompat.hasPrimaryNetwork(player)) {
            return true;
        }
