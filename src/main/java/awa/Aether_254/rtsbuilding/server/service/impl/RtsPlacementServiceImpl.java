...
import awa.Aether_254.rtsbuilding.server.pipeline.context.PlaceContext;
...
import awa.Aether_254.rtsbuilding.server.service.api.PlacementService;
...
import awa.Aether_254.rtsbuilding.server.workflow.model.RtsWorkflowStatus;
 ...
 *
 * <p>该实现类通过 {@link awa.Aether_254.rtsbuilding.server.pipeline.core.PipelineRegistry}
 * 执行放置流程：
 * <ul>
 ...
 * 当工作流不可用时回退到直接入队（{@link awa.Aether_254.rtsbuilding.server.service.placement.RtsPlacementBatch}）。
 */
public final class RtsPlacementServiceImpl implements PlacementService {
    ...
    public int submitPendingPlacement(ServerPlayer player) {
        ...
        int count = awa.Aether_254.rtsbuilding.server.service.RtsPendingPlacementService
    ...
    private boolean canRotateBlock(ServerPlayer player, BlockPos pos) {
        ...
        RtsStorageSession session = registry.session().getIfPresent(player);
        if (session == null
                || session.mode != awa.Aether_254.rtsbuilding.common.build.BuilderMode.ROTATE
                || player.isSpectator()
                || !player.mayBuild()
