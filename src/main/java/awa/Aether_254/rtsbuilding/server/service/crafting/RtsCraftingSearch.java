...
import awa.Aether_254.rtsbuilding.util.RtsPinyinSearch;
 ...
 * </ul>
 *
 * <p>搜索状态存储在 {@link awa.Aether_254.rtsbuilding.server.storage.session.RtsBrowserState} 中，
...
public final class RtsCraftingSearch {
    ...
    public static void requestCraftables(ServerPlayer player, RtsStorageSession session, String search,
        ...
        for (RecipeHolder<?> rawHolder : player.level().getServer().getRecipeManager().getRecipes()) {
            if (!(rawHolder.value() instanceof CraftingRecipe)) {
                continue;
            }
    ...
    private static List<AvailableCraftItem> snapshotAvailableCraftItems(
        ...
        boolean includePlayerMainInventory = session != null
                ...
                && player != null
                && !(player.containerMenu instanceof awa.Aether_254.rtsbuilding.server.menu.RtsCraftTerminalMenu);
    ...
    static boolean supportsWorkbenchCraftPanelRecipe(CraftingRecipe recipe) {
        ...
        if (recipe instanceof ShapedRecipe shaped) {
            ...
            if (shapeless.placementInfo().ingredients().isEmpty() || shapeless.placementInfo().ingredients().size() > 9) {
                return false;
            }
        } else {
